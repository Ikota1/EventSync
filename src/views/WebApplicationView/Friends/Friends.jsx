...
const Friends = () => {
  ...
  useEffect(() => {
    ...
    const handleRemoveFriendBtnClick = async (userToDeleteID) => {
          ...
          userFriends.map((user) => (
            <div key={user.uid} className="w-full max-w-sm pt-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
              ...
              <div className="flex justify-end px-4 pt-4">
                <button onClick={() => handleRemoveFriendBtnClick(user.uid)} className="bg-blue-500 text-white px-2 py-1 rounded mb-4">Remove Friend</button>
              </div>
            </div>
          ))
        )}
